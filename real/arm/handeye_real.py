#!/usr/bin/env python3
"""Hand-eye на живой руке: куб в губках, тур по позам, X = камера->база.

Порт сим-процедуры (mlsim/test_handeye.py): случайные позы с проверкой пути
floor-guard'ом, поиск ракурса роллом (куб размечен со всех сторон), осадка и
двойной замер против недоседания, сегментный решатель Парка-Мартина со
слип-фильтром по инварианту углов и trim'ом. Камера C920 с реальными
интринсиками (real/calib/c920_intrinsics.json, с дисторсией).
"""
...
CAM_JPG = Path("/tmp/roboom_cam/latest.jpg")
N_TARGET = 40
POSE_LO = np.array([-0.9, -1.1, 0.2, -0.6, -1.2])
POSE_HI = np.array([0.9, 0.1, 1.4, 0.9, 1.2])
OUT = _ROOT / "real" / "calib" / "handeye_c920.json"

calib = json.loads((_ROOT / "real" / "calib" / "c920_intrinsics.json").read_text())
K = np.array(calib["K"])
    ...
    for seg in segments:
        ...
        for i in range(len(seg) - 1):
            for j in (i + 1, i + 2):
                if j >= len(seg):
                    continue
                A = Fs[i] @ np.linalg.inv(Fs[j])
                B = Cs[i] @ np.linalg.inv(Cs[j])
...
def main():
    ...
    try:
        ...
        cam_pos = X[:3, 3]
        print(f"X (камера->база): позиция камеры в базе {np.round(cam_pos, 3)}, "
              f"медианная невязка {resid:.1f} мм")
        OUT.write_text(json.dumps({
            "T_cam2base": X.tolist(), "pairs": len(pairs),
